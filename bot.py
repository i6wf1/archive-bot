...
# ─── Modals ───────────────────────────────────────────────
class RateItemModal(discord.ui.Modal, title="تقييم العمل بالنج"):
...
class AddItemModal(discord.ui.Modal, title="إضافة عمل للستة"):
    item_title = discord.ui.TextInput(label="اسم الفيلم أو المسلسل (لالبحث)", placeholder="مثال: Iron Man", required=True)
...
class ChooseItemToManageModal(discord.ui.Modal, title="اختيار فيلم لإدارته"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # فتح لوحة التحكم المخصصة لهذا الفيلم المحدد فوراً
        item = items[num - 1]
        embed = discord.Embed(
...
class EditItemDetailsModal(discord.ui.Modal, title="تعديل تفاصيل العمل"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # تحديث البيانات الأساسية للفيلم
        ...
        # إعادة الإدراج في الترتيب والمكان الجديد المختار بكل سهولة
...
# ─── Manage Dashboard View (قائمة الترس الزرقاء المحدثة) ────
class ManageDashboardView(discord.ui.View):
    ...
    @discord.ui.button(label="حذف اللستة كاملة", style=discord.ButtonStyle.primary, row=1)
