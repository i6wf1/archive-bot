...
async def generate_grid_image(items: list) -> BytesIO:
    # فلترة الأعمال التي تحتوي على بوستر صالح فقط
    ...
    poster_images = []
    async with aiohttp.ClientSession() as session:
        for url in valid_posters[:15]: # حد أقصى 15 بوستر للرسالة الواحدة لضمان الأداء
            try:
                async with session.get(url, timeout=5) as resp:
                    if resp.status == 200:
                        img_data = await resp.read()
                        img = Image.open(BytesIO(img_data)).convert("RGB")
                        # توحيد حجم البوسترات (عرض 200 وارتفاع 300)
    ...
    columns = 5  # عدد البوسترات في كل سطر تماماً مثل طلبك
    rows = (len(poster_images) + columns - 1) // columns
    
    # أبعاد الصورة الكبيرة الشاملة
    grid_width = columns * 200 + (columns - 1) * 10
    grid_height = rows * 300 + (rows - 1) * 15
    
    grid_img = Image.new("RGB", (grid_width, grid_height), color=(47, 49, 54)) # خلفية متناسقة مع ديسكورد
...
async def update_view_panel(interaction: discord.Interaction, list_name: str, items: list, list_names: list[str]):
    ...
    # تعديل الرسالة الأساسية لتظهر اللستة المرئية الجديدة
    if file:
...
class ListView(discord.ui.View):
    def __init__(self, current_list_name: str, items: list, is_manager: bool, list_names: list[str]):
        ...
        # الصف الأول: أزرار التحكم الأساسية
        self.add_item(ManageButton(current_list_name, list_names))
        self.add_item(HomeButton())
        self.add_item(RateButton(current_list_name, list_names))

        # الصف الثاني وما بعده: أزرار مرقمة لكل فيلم بناءً على الترتيب في الصورة
        for idx, item in enumerate(items[:15]): # عرض أول 15 عمل تماشياً مع الأزرار والبوسترات
    ...
    def make_item_callback(self, idx: int):
        async def callback(interaction: discord.Interaction):
            ...
            # بناء إمبيد تفاصيل العمل الفردي الذي يظهر عند الضغط على الزر
            ...
            # إرسال التفاصيل كرسالة مخفية (Ephemeral) للشخص الذي ضغط فقط كي لا تخرب مظهر اللستة العامة
...
class HomeButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction): await return_to_main_panel(interaction)
...
class WonderlandBot(commands.Bot):
    ...
    async def setup_hook(self): print("✨ [Bot] تم تشغيل البوت بنجاح واحترافية!")
...
async def cmd_panel(interaction: discord.Interaction):
    if not can_manage(interaction.user): return
    await interaction.response.defer(ephemeral=True)
