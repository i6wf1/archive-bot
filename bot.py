...
class RenameListModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not new_list_name:
            await interaction.followup.send("❌ الاسم لا يمكن أن يكون فارغاً.", ephemeral=False)
            return

        data = load_data()
        if new_list_name in data["lists"]:
            await interaction.followup.send("❌ يوجد قائمة أخرى تحمل هذا الاسم بالفعل.", ephemeral=False)
            return
        ...
        if self.current_list_name in data["lists"]:
            ...
            await interaction.followup.send(f"✅ تم تغيير اسم اللستة بنجاح إلى **{new_list_name}**!", ephemeral=False)
...
class RateItemModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("❌ يرجى إدخل أرقام صحيحة، والتقييم بين 1 و 5 نجوم.", ephemeral=False)
            return

        data = load_data()
        items = data["lists"].get(self.list_name, {}).get("items", [])
        if num < 1 or num > len(items):
            await interaction.followup.send("❌ رقم الفيلم غير موجود.", ephemeral=False)
            return
        ...
        await interaction.followup.send(f"✅ تم تسجيل تقييمك بنجاح!", ephemeral=False)
...
class AddItemModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.list_name in data["lists"]:
            ...
            view = ListView(self.list_name, items, can_manage(interaction.user), self.list_names, data["lists"])
            await update_global_panel(interaction, embeds, view)
            await interaction.followup.send(f"✅ تمت إضافة العمل بنجاح وبسرعة فائقة عبر OMDB!", ephemeral=False)
...
class EditItemDetailsModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("❌ الترتيب يجب أن يكون رقماً صحيحاً.", ephemeral=False)
            return
        ...
        await interaction.followup.send("✅ تم تعديل البيانات وإعادة الترتيب بنجاح!", ephemeral=False)
...
class ItemEditorDashboard(discord.ui.View):
    ...
    async def delete_item(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("✅ تم حذف العمل بنجاح وتحديث اللستة!", ephemeral=False)
...
class ItemDropdownSelector(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if index >= len(items):
            await interaction.response.send_message("❌ حدث خطأ، لم يتم العثور على العمل المختار.", ephemeral=False)
            return
...
class ReorderListsModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # نزيل الأرقام في البداية لو المستخدم تركها (مثلاً "1. MARVEL" -> "MARVEL")
        new_order = []
        for line in raw_lines:
            line = line.strip()
            if not line:
                continue
            # إزالة رقم الترتيب في البداية لو موجود
            if line[0].isdigit():
                parts = line.split(".", 1)
                if len(parts) == 2:
                    line = parts[1].strip()
        ...
        # التحقق أن الأسماء صحيحة
        invalid = [n for n in new_order if n not in existing]
        missing = [n for n in existing if n not in new_order]

        if invalid:
            await interaction.followup.send(f"❌ هذه الأسماء غير موجودة: {', '.join(invalid)}", ephemeral=False)
            return
        if missing:
            # نضيف المفقودين في النهاية تلقائياً
            new_order += missing

        # إعادة بناء الـ dict بالترتيب الجديد
        reordered = {name: data["lists"][name] for name in new_order}
        ...
        await interaction.followup.send("✅ تم تغيير ترتيب اللستات بنجاح!", ephemeral=False)
        # تحديث الباول الرئيسي
...
class ListView(discord.ui.View):
    ...
    def make_navigation_callback(self, name: str):
        async def callback(interaction: discord.Interaction):
            ...
            if not lst:
                await interaction.response.send_message("القائمة غير موجودة.", ephemeral=False)
                return
            items = lst.get("items", [])
...
class ManageButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if not can_manage(interaction.user):
            await interaction.response.send_message("⚠️ عذراً، لوحة الإدارة مخصصة للمشرفين فقط!", ephemeral=False)
            return
        try:
            embed = discord.Embed(
                title=f"إدارة — {self.list_name}",
...
async def refresh_panel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    if old_msg_id:
        try:
            ...
            await interaction.followup.send("🔄 تم تحديث لوحة التحكم القديمة بنجاح!", ephemeral=False)
            return
    ...
    save_data(data)
    await interaction.followup.send("✅ تم إنشاء لوحة تحكم جديدة بنجاح!", ephemeral=False)
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    ...
    try:
        if interaction.response.is_done():
            await interaction.followup.send(f"❌ حدث خطأ داخلي: {error}", ephemeral=False)
        else:
            await interaction.response.send_message(f"❌ حدث خطأ داخلي: {error}", ephemeral=False)
    except Exception:
        pass
...
async def cmd_panel(interaction: discord.Interaction):
    if not can_manage(interaction.user):
        await interaction.response.send_message("⚠️ خطأ: هذا الأمر مخصص للإدارة فقط.", ephemeral=False)
        return
    await interaction.response.defer(ephemeral=False)
    try:
        ...
        traceback.print_exc()
        await interaction.followup.send("❌ حدث خطأ غير متوقع أثناء تحديث اللوحة.", ephemeral=False)
...
async def cmd_list_create(interaction: discord.Interaction, name: str):
    if not can_manage(interaction.user):
        await interaction.response.send_message("⚠️ خطأ: ليس لديك صلاحية إنشاء قوائم جديدة.", ephemeral=False)
        return
    await interaction.response.defer(ephemeral=False)
    name = name.strip()
    data = load_data()
    if name in data["lists"]:
        await interaction.followup.send("القائمة موجودة مسبقاً.", ephemeral=False)
        return
    data["lists"][name] = {"description": "", "items": []}
    save_data(data)
    await interaction.followup.send(f"تم إنشاء القائمة **{name}** بنجاح!", ephemeral=False)
