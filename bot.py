...
MAIN_IMAGE_PATH = "4fa44a84a41e84983b8a08e4f4b1eb93.png"
...
# دالة إعادة تعيين الواجهة الرئيسية عند انتهاء الوقت (دقيقة بدون تفاعل)
async def reset_to_main_panel_msg(message: discord.Message):
    try:
        data = load_data()
        ...
        if os.path.exists(MAIN_IMAGE_PATH):
            file = discord.File(MAIN_IMAGE_PATH, filename="main_panel.png")
            embed.set_image(url="attachment://main_panel.png")
            files = [file]
            
        view = PanelView(list_names, data["lists"])
        await message.edit(embeds=[embed], view=view, attachments=files)
    except Exception as e:
        print(f"Error on timeout panel reset: {e}")
...
class ItemEditorDashboard(discord.ui.View):
    ...
    async def on_timeout(self):
        if self.message:
            await reset_to_main_panel_msg(self.message)
    ...
    async def delete_item(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        view = ListView(self.list_name, items, can_manage(interaction.user), self.list_names, data["lists"])
        await update_global_panel(interaction, embeds, view)
        await interaction.followup.send(content="✅ تم حذف الفيلم بنجاح وتحديث اللستة.", ephemeral=True)
        ...
        view_manage = ManageDashboardView(self.list_name, self.list_names)
        await interaction.message.edit(embeds=[embed], view=view_manage, attachments=[])
        view_manage.message = interaction.message
    ...
    async def back_to_dashboard(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        view = ManageDashboardView(self.list_name, self.list_names)
...
class ItemDropdownSelector(discord.ui.Select):
    def __init__(self, list_name: str, list_names: list[str], options: list):
        super().__init__(
            placeholder="تعديل محتوى اللستة",
            min_values=1,
            max_values=1,
            options=options,
            row=2
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view = ItemEditorDashboard(self.list_name, self.list_names, index + 1, item)
        await interaction.response.edit_message(embeds=[embed], view=view, attachments=[])
        view.message = await interaction.original_response()
...
class ManageDashboardView(discord.ui.View):
    def __init__(self, list_name: str, list_names: list[str]):
        ...
        if options:
            ...
            disabled_select = discord.ui.Select(
                placeholder="تعديل محتوى اللستة", 
                disabled=True, 
                options=[discord.SelectOption(label="القائمة فارغة تماماً", value="empty_fallback")],
                row=2
            )
    ...
    @discord.ui.button(label="تخصيص اللستة", style=discord.ButtonStyle.primary, row=0)
    async def modify_group_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.stop()
        embed = discord.Embed(
            title=f"لوحة التخصيص — {self.list_name}",
            description="اختر الإجراء المطلوب لتعديل اسم اللستة، أو إزالتها نهائياً.",
            color=0xd3beab
        )
        view = ListModificationSubView(self.list_name, self.list_names)
        await interaction.response.edit_message(embeds=[embed], view=view, attachments=[])
        view.message = await interaction.original_response()
...
async def return_to_main_panel(interaction: discord.Interaction):
    ...
    view = PanelView(list_names, data["lists"])
    await interaction.response.edit_message(embeds=[embed], view=view, attachments=files)
...
async def refresh_panel(guild: discord.Guild, channel: discord.TextChannel):
    ...
    if old_msg_id:
        try:
            ...
            await old_msg.edit(embeds=[embed], view=view, attachments=files)
            return
        except discord.NotFound:
            pass
