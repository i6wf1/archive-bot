...
# Helper لتحديث الرسالة تلقائياً عند انتهاء الوقت (دقيقة بدون تفاعل)
...
class EditItemDetailsModal(discord.ui.Modal, title="تعديل تفاصيل العمل"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("❌ التترتيب يجب أن يكون رقماً صحيحاً.", ephemeral=True)
            return
...
class ItemEditorDashboard(discord.ui.View):
    ...
    async def delete_item(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.message.edit(embeds=[embed], view=view_manage)
...
class ManageDashboardView(discord.ui.View):
    def __init__(self, list_name: str, list_names: list[str]):
        ...
        # بناء القائمة المنسدلة بناءً على المحتويات الحالية لمنع الـ failed interaction
        data = load_data()
