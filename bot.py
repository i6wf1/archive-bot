...
OMDB_API_KEY = "911582c4"  # مفتاح OMDb الخاص بك (صحيح وضابط)
...
# ─── OMDb Official Fetcher (تم إصلاح جلب البوستر والسنة) ───
def fetch_official_theatrical_details(query: str) -> dict:
    try:
        encoded_query = urllib.parse.quote(query)
        # استدعاء الرابط باستخدام مفتاح OMDb الخاص بك
        url = f"http://www.omdbapi.com/?apikey={OMDB_API_KEY}&t={encoded_query}"
        req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=5) as response:
            data = json.loads(response.read().decode('utf-8'))
            if data.get("Response") == "True":
                # هنا قمنا بتحويل الحروف الكابيتال من OMDb إلى سمول لتتوافق مع نظام الـ Embeds الخاص بك
...
# ─── تجربة الحذف الجماعي الاحترافية والقائمة المنسدلة ─────────
