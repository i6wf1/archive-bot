...
OMDB_API_KEY = "911582c4"  # تم إلغاء TMDB واستبداله بـ OMDb وبمفتاحك الخاص
...
# ─── OMDb Official Poster & Year Fetcher ──────────────────
def fetch_official_theatrical_details(query: str) -> dict:
    try:
        encoded_query = urllib.parse.quote(query)
        url = f"http://www.omdbapi.com/?apikey={OMDB_API_KEY}&t={encoded_query}"
...
# ─── تجربة الحذف الجماعي الاحترافية بسلاسة (Select Menu) ───────
class BulkDeleteSelect(discord.ui.Select):
    def __init__(self, list_name: str, items: list, list_names: list[str]):
        self.list_name = list_name
        self.list_names = list_names
        
        # إنشاء قائمة الخيارات بناءً على الأعمال الموجودة
        options = [
            discord.SelectOption(
                label=f"{i+1}. {get_title(item)[:50]}", 
                value=str(i),
                description=f"سنة الإنتاج: {get_year(item)}" if get_year(item) else None
            ) for i, item in enumerate(items[:25]) # ديسكورد يدعم حد أقصى 25 خيار بالقائمة المنسدلة
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        # تحويل القيم المحددة لأرقام وترتيبها تنازلياً للحذف الآمن
...
class ListModificationSubView(discord.ui.View):
    ...
    async def bulk_delete_items(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not items:
            ...
            return
        await interaction.response.send_message("تفضل يا وحش، حدد من القائمة أدناه الأعمال التي تود إزالتها (يمكنك اختيار عمل أو أكثر):", view=BulkDeleteView(self.list_name, items, self.list_names), ephemeral=True)
