...
class EditItemDetailsModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # إرجاع المشرف تلقائياً للوحة التحكم المحدثة لمتابعة عمله بسلاسة واحترافية
...
# ─── Specialized Item Editor Dashboard (لوحة الخيارات للفيلم المختار) ───
class ItemEditorDashboard(discord.ui.View):
    ...
    async def delete_item(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # إرجاع المشرف تلقائياً للوحة الإدارة الرئيسية بعد الحذف
        mgr_embed = discord.Embed(
            title=f"إدارة — {self.list_name}",
            description="التحكم الكامل والذكي بمحتوى وتعديل القائمة، ترتيب الأعمال، تغيير اسم اللستة أو حذفها.",
            color=0xd3beab
...
# ─── Dropdown Component (مكون القائمة المنسدلة الخارق) ───────
class ItemDropdownSelector(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # التنقل الفوري اللطيف لخيارات التعديل
...
# ─── Unified Management Dashboard View (لوحة التحكم الشاملة والجديدة) ───
class ManageDashboardView(discord.ui.View):
    def __init__(self, list_name: str, list_names: list[str]):
        ...
        # جلب الأعمال لبناء القائمة المنسدلة ديناميكياً
        data = load_data()
        items = data["lists"].get(list_name, {}).get("items", [])
        
        options = []
        for i, item in enumerate(items[:25]): # حد ديسكورد الأقصى للقوائم المنسدلة
            title = get_title(item)
            year = get_year(item)
