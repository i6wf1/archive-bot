...
class RateItemModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        view = ListView(self.list_name, items, can_manage(interaction.user), self.list_names, data["lists"], message=interaction.message)
...
class EditItemDetailsModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.message.edit(embeds=[mgr_embed], view=ManageDashboardView(self.list_name, self.list_names, message=interaction.message))

# ─── Specialized Item Editor Dashboard ────────────────────
class ItemEditorDashboard(discord.ui.View):
    def __init__(self, list_name: str, list_names: list[str], num: int, item: dict, message: discord.Message = None):
    ...
    async def back_to_mgr(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(embeds=[embed], view=ManageDashboardView(self.list_name, self.list_names, message=interaction.message))
    ...
    async def on_timeout(self):
        if self.message:
            try:
                ...
                lines = "\n".join(f"🔴 **{k.upper()}** —  `{len(v.get('items', []))} Entries`" for k, v in data["lists"].items()) if list_names else "لا توجد قوائم متوفرة حالياً."
                embed = discord.Embed(title="Wonderland Lists", description=f"\n{lines}\n", color=0xd3beab)
                view = PanelView(list_names, data["lists"])
                await self.message.edit(embeds=[embed], view=view)
            except Exception:
                pass
...
class ItemDropdownSelector(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(embeds=[embed], view=ItemEditorDashboard(self.list_name, self.list_names, index + 1, item, message=interaction.message))

# ─── Unified Management Dashboard View ────────────────────
class ManageDashboardView(discord.ui.View):
    def __init__(self, list_name: str, list_names: list[str], message: discord.Message = None):
        super().__init__(timeout=120.0)
        self.list_name = list_name
        self.list_names = list_names
        self.message = message
...
class ListView(discord.ui.View):
    def __init__(self, current_list_name: str, items: list, is_manager: bool, list_names: list[str], all_lists_data: dict, message: discord.Message = None):
        super().__init__(timeout=120.0)
        self.current_list_name = current_list_name
        self.list_names = list_names
    ...
    def make_navigation_callback(self, name: str):
        async def callback(interaction: discord.Interaction):
            ...
            view  = ListView(name, items, can_manage(interaction.user), self.list_names, data["lists"], message=interaction.message)
            await interaction.response.edit_message(embeds=embeds, view=view)
        return callback
