...
import os
import urllib.parse
import aiohttp  # تم التحويل إلى المكتبة اللامركزية لمنع تجميد البوت نهائياً
...
# ─── OMDB Async Fetcher (علاج مشكلة لقطة الشاشة) ──────────
async def fetch_official_theatrical_details(query: str) -> dict:
    print(f"🔍 [OMDb] جاري البحث اللامتزامن عن العمل: '{query}'")
    try:
        encoded_query = urllib.parse.quote(query.strip())
        url = f"https://www.omdbapi.com/?apikey={OMDB_API_KEY}&t={encoded_query}"
        
        # استخدام الاتصال الذكي لضمان عدم حدوث تجميد أو تعليق
...
class AddItemModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # تأجيل الاستجابة فوراً لمنع حدوث تعليق ديسكورد
        await interaction.response.defer(ephemeral=True)
        
        # جلب البيانات بطريقة غير حاصرة (Async await) لضمان استقرار الواجهة بنسبة 100%
