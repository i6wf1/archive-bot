...
def build_separate_embeds(list_name: str, items: list) -> list[discord.Embed]:
    if not items:
        embed = discord.Embed(
            ...
            color=0x1e1f22  # تم التغيير للون الخلفية الموحد لاختفاء الخط تماماً
        )
        return [embed]
    ...
    for i, item in enumerate(items[:10]):
        ...
        # تم ضبط لون الـ Embed ليتطابق مع الخلفية الداكنة
...
class RenameListModal(discord.ui.Modal, title="تغيير اسم اللستة"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.current_list_name in data["lists"]:
            # نقل البيانات للاسم الجديد وحذف القديم
            data["lists"][new_list_name] = data["lists"].pop(self.current_list_name)
            save_data(data)
            
            # العودة التلقائية للواجهة الرئيسية وتحديث اللستات
...
class ManageDashboardView(discord.ui.View):
    ...
    @discord.ui.button(label="إدارة الأفلام", style=discord.ButtonStyle.primary, row=0)
...
class ManageButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description="التحكم الكامل والذكي بمحتوى وتعديل القائمة وترتيب الأعمال وتغيير اسم اللستة.",
            color=0x1e1f22
        )
