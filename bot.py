...
OMDB_API_KEY = "911582c4"  # مفتاحك المعتمد
...
# ─── OMDb Official Fetcher (مصحح بالكامل ومزود بنظام فحص تلقائي) ───
def fetch_official_theatrical_details(query: str) -> dict:
    print(f"🔍 [OMDb] جاري البحث عن العمل: '{query}'")
    try:
        encoded_query = urllib.parse.quote(query.strip())
        # تعديل الرابط إلى https لضمان استجابة السيرفر الفورية
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            # طباعة الاستجابة في الكونسول لتراقب النتيجة بنفسك أثناء التشغيل
            ...
            if data.get("Response") == "True":
                ...
                year = data.get("Year", "")
                print(f"✅ [OMDb] تم العثور على البيانات بنجاح! سنة الإنتاج: {year}")
                return {"title": title, "poster": poster, "year": year, "ratings": {}}
            else:
                print(f"❌ [OMDb] السيرفر لم يجد العمل. السبب: {data.get('Error', 'Unknown Error')}")
                print("💡 تلميح: تأكد من كتابة اسم العمل باللغة الإنجليزية حصراً.")
    except Exception as e:
        print(f"🚨 [OMDb] خطأ فادح أثناء طلب البيانات: {e}")
...
class ListModificationSubView(discord.ui.View):
    ...
    async def bulk_delete_items(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not items:
            ...
            return
        await interaction.response.send_message("حدد من القائمة أدناه الأعمال التي تود إزالتها (يمكنك اختيار عمل أو أكثر):", view=BulkDeleteView(self.list_name, items, self.list_names), ephemeral=True)
